...
Из корня репозитория, по одной команде в PowerShell. Пока локальные AI-файлы не закоммичены в официальный репозиторий, чистый клон не содержит весь описанный здесь AI-сценарий; воспроизводимость после публикации изменений требуется проверить отдельно.
