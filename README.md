...
Из корня репозитория, по одной команде в PowerShell. AI-модуль и проверки находятся в официальном репозитории. Полный запуск из чистого клона по этим инструкциям ещё не проверен; локальные проверки выполнялись на отдельной тестовой PostgreSQL.
